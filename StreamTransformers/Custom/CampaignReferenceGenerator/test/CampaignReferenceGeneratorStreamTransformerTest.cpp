...
void CampaignReferenceGeneratorStreamTransformerTest::testCampaignReferenceGenerator()
{
	...
	inputStream << "campaign_id,ahead_schedule_daily_value,on_schedule_daily_value,behind_schedule_daily_value,cost_per_unit,click_hard_target,action_hard_target,indexed_impressions,indexed_clicks,indexed_actions" << std::endl
	 ...
	 << "172785,278,277,277,ABC,30,10,45,10,10" << std::endl; // Non-Integer CPU Value
	
	boost::shared_ptr<std::stringstream> outputStream  = GenerateCampaignReference( inputStream, parameters );
	...
	expected<< "172780,0,277.000000,277.000000,277.000000" <<std::endl
			...
			CPPUNIT_ASSERT_EQUAL( expected.str(), outputStream->str() );
...
void CampaignReferenceGeneratorStreamTransformerTest::testMissingCampaignId()
{
	...
	std::stringstream expected;
			// campaign 172781
	expected<<"campaign_id,reference_type,reference_value,behind_tolerance,ahead_tolerance" << std::endl
			<< "172780,0,277.000000,277.000000,277.000000" <<std::endl
			<< "172780,1,240.869565,240.869565,240.869565" << std::endl
			<< "172780,3,30.000000,," << std::endl
			<< "172780,4,10.000000,," << std::endl
			<< "172780,5,10.450000,," << std::endl
