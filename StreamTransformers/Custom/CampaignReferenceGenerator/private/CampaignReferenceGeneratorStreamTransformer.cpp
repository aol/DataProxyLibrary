...
#include "PreCampaignReferenceDatum.hpp"
...
boost::shared_ptr< std::stringstream > GenerateCampaignReference( std::istream& i_rInputStream, const std::map< std::string, std::string >& i_rParameters )
{
	...
	CampaignReferenceStreamWriter writer( *pResult );
	...
	//CampaignReferenceStreamWriter writer( campaignReferenceDataContainer );
	//writer.Write( *pResult );
	return pResult;
}
