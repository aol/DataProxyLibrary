//
// FILE NAME:	   $HeadURL: svn+ssh://svn.cm.aol.com/advertising/adlearn/gen1/trunk/lib/cpp/DataProxy/private/BlackoutStreamTransformer.cpp $
//
...
namespace
{
	...
	boost::shared_ptr< std::istream > ApplyBlackouts( boost::shared_ptr< std::istream > i_pInputStream, const std::map<std::string, std::string >& i_rParameters )
	{
...
boost::shared_ptr<std::istream> BlackoutStreamTransformer::TransformInput( boost::shared_ptr<std::istream> i_pInputStream, const std::map<std::string, std::string >& i_rParameters )
{
	return ApplyBlackouts(i_pInputStream, i_rParameters);
}
