// FILE NAME:       $RCSfile: DataProxyClientTest.cpp,v $
//
// REVISION:        $Revision$
//
...
namespace
{
	...
	std::string s_Error("");
	TestableDataProxyClient s_DataProxyClient;
	std::string s_LoadData;
	std::string s_LoadName;
	...
	void RunLoop()
	{
		...
		try
		{
			for( int i=0; i<10 && s_Error.empty(); ++i )
			{
				std::stringstream result;
				s_DataProxyClient.Load( s_LoadName, parameters, result );
				CPPUNIT_ASSERT_EQUAL( s_LoadData, result.str() );
			}
		}
...
void DataProxyClientTest::testMultithread()
{
	s_Error = "";
	s_LoadName = "name1";
	std::stringstream data;
	...
	{
		data << i << "\tthis is some data that will be returned!";
	}
	s_LoadData = data.str();
	...
	file << "<DPLConfig>" << std::endl
		 << "  <DataNode name=\"" << s_LoadName << "\" type=\"dummy\" />" << std::endl
		 << "</DPLConfig>" << std::endl;
	file.close();

	MockNodeFactory factory;
	factory.SetDataToReturn( s_LoadName, s_LoadData );
	CPPUNIT_ASSERT_NO_THROW( s_DataProxyClient.Initialize( configFileSpec, factory ) );

	std::vector< boost::shared_ptr< boost::thread > > threads( 50 );
