//
// FILE NAME:       $HeadURL: svn+ssh://sstrick@svn.cm.aol.com/advertising/adlearn/gen1/trunk/lib/cpp/DataProxy/StreamTransformers/Validator/test/ValidateStreamTransformerTest.cpp $
//
...
void ValidateStreamTransformerTest::testMissingParameters()
{
	...
	boost::shared_ptr< std::istream > pInputStream( pRawInputStream );;
	*pRawInputStream << "data1,data2" << std::endl;

	std::map< std::string, std::string > parameters;
	parameters["timeout"] = "5";

	CPPUNIT_ASSERT_THROW_WITH_MESSAGE( ValidateStreamTransformer().TransformInput( pInputStream, parameters ), ValidateStreamTransformerException,
		".*\\.cpp:\\d+: No validation properties specified" );

	parameters["discardIf"] = "data2<3";
	parameters.erase( "timeout" );
	CPPUNIT_ASSERT_THROW_WITH_MESSAGE( ValidateStreamTransformer().TransformInput( pInputStream, parameters ), TransformerUtilitiesException,
...
void ValidateStreamTransformerTest::testValidate_DiscardModifyRecords()
{
	std::stringstream* pRawInputStream = new std::stringstream;
	boost::shared_ptr< std::istream > pInputStream( pRawInputStream );;
	*pRawInputStream << "col1, 2 c o\tl 2.:;!@#$%^&*() ,col3,NR,col5,col6,col7" << std::endl	// second column accessed as "_2col2" (illegal var chars), fourth as "_NR" (NR is built-in awk variable)
	...
	boost::shared_ptr< std::istream > pResult = ValidateStreamTransformer().TransformInput( pInputStream, parameters );
...
void ValidateStreamTransformerTest::testValidate_Fail()
{
	std::stringstream* pRawInputStream = new std::stringstream;
	boost::shared_ptr< std::istream > pInputStream( pRawInputStream );;
	*pRawInputStream << "col1,col2,col3,col4,col5,col6,col7" << std::endl
				<< "11,12,13,14,15,16,17" << std::endl
	...
	CPPUNIT_ASSERT_THROW_WITH_MESSAGE( ValidateStreamTransformer().TransformInput( pInputStream, parameters ), ValidationFailedException,
